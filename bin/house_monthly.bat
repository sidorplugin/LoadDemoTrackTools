...
REM Для автоматизации рекомендуется создать как ежемесячную задачу в планировщике;
...
rem python deejayde_tracklist.py djde.bin -g house -f %FROM_DATE% -t %TO_DATE% -b 1
rem python juno_tracklist.py juno.bin -g house -f %FROM_DATE% -t %TO_DATE%
rem python unique_list.py djde.bin juno.bin
